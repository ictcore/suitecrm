...
class CE_custom_exportViewCE_config_sp extends SugarView {
    ...
    function display() {
        ...
        $select_query_sub = "SELECT subpanel,module FROM custom_export_subpanel_tbl";
        $saved_subpanel_result = $db->query($select_query_sub);
        ...
        $html = "<div style='font-size: 12pt;font-weight: bold;padding-bottom: 8px;'>Enable custom export feature for module sub-panels.</div>";
        ...
        $html .= "<form name='exportSubpanelForm' method='post' action='index.php?module=CE_custom_export&action=saveExportSubpanel'>
                ";
        ...
        $all_enableModules = is_array($all_enableModules) ? $all_enableModules : array($all_enableModules);
        foreach ($all_enableModules as $module) {
            if (!in_array($module, $excluded_modules)) {
                ...
                if (!empty($display_SubpanelsInMod)) {
                    $html .= "
                   <div id='$module' class='module-row'>
                        <label class='label' style='color:#000;width: 100%;'>{$app_list_strings['moduleList'][$module]}</label>";
                    $html .= "<ul>";
                    $targetModuleLang = return_module_language('en_us', $module);
                    foreach ($display_SubpanelsInMod as $title => $subpanel_setup) {
                        $subpanelArr = explode(",", $saved_subpanel_array[$module]);
                        if (!in_array($subpanel_setup, $excluded_all_subpanels) && !empty($title)) {
                            if (in_array($subpanel_setup, $subpanelArr)) {
                            ...
                            $html .= "<li><input type='checkbox' name='chksubpanel[$module][]' value='{$subpanel_setup}' {$checked}>{$sub_label}</li>";
                    }
                    }
                    $html .= " </ul>";
                    $html .= "</div>";
